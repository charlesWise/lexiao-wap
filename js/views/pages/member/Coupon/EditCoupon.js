...
export default class CouponEdit extends ScreenComponent {
    ...
    _getDetail() {
        ...
        api.viewCoupon({
            ...
            let merchant_ids = data.store_id;
            this.setState({
                detail: data,
                coupon_id: data.id,
                type: type,
    ...
    _doSubmit(status) {
        ...
        let merchant_ids = this.state.mid//适用门店 id
    ...
    _FixedBottomControl(detail) {
        ...
        if (detail.status == 1 || detail.status == 3 || detail.status == 4){
            return (
