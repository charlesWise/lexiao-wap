        ...
        return (
            <div className='coupon tabs -notborder -fixed'>
                ...
                <div className='fixed-bottom'>
                    <a className='bd-coupon-add' href="#/bd/coupon/add">添加福利券</a>
                </div>
            </div>
        )
    }
...
class ListItem extends ScreenComponent {
    ...
    _goDetail(data) {
        this.getScreen().getNavigation().navigate('BDCouponDetail', {
            coupon_id: data.coupon_id
        })
    }

    _goEdit(data) {
        this.getScreen().getNavigation().navigate('BDCouponEdit', {
            coupon_id: data.coupon_id
        })
    }

    _goCheck(data) {
        this.getScreen().getNavigation().navigate('BDCouponCheck', {
            coupon_id: data.coupon_id
        })
    }
    ...
    render() {
        ...
        let info = user.info || {};
        let auth = user.auth || [];
        let index = _.findIndex(auth, function (o) { return o['code'] == 'MERCHANT_WEAL_AUDIT'; });//福利卷审核
        return (
            <div className={this._getBgClass(data)} onClick={(e) => {
                ...
                <div className='main'>
                    <h3 className='title'>{data.merchant_name}{data.type == 2 ? '满减券' : '代金券'}</h3>
                    <p className='time'>有效期至：{data.use_time_end}</p>
                    <dl className='sum'>
                        <dt>¥<span>{data.discount_amount}</span></dt>
                        <dd>{
                            data.type == 1 ? '' : `满${data.need_amount}可用`
                        }</dd>
                    </dl>
                </div>
                <div className='prop'>
                    <div className='prop-item'>发行量：<span>{data.publish_num || 0}</span>张</div>
                    <div className='prop-item'>已领取：<span>{data.received_num || 0}</span>张</div>
